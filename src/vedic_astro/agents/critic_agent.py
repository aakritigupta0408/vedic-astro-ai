...
class CriticAgent:
    ...
    def _parse_result(self, raw: str) -> CriticResult:
        try:
            ...
            ca  = float(data.get("classical_accuracy", 0.5))
            qr  = float(data.get("query_relevance", 0.5))
            sp  = float(data.get("specificity", 0.5))
            co  = float(data.get("consistency", 0.5))
            ...
            # Parsing failure → neutral score (0.5) so the reading is re-evaluated,
            # not silently passed. 0.5 is below the default 0.75 threshold → triggers reviser.
            logger.warning("Critic parse failed (%s); raw=%r — marking FAIL for revision", exc, raw[:200])
            return CriticResult(
                ...
                consistency=0.5,
                composite_score=0.5,
                issues=["Critic output could not be parsed — revision triggered"],
                passed=False,
            )
